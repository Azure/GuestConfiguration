    ...
    InModuleScope -ModuleName 'GuestConfiguration' {
        ...
        foreach ($OS in @('Windows','Linux')) {
            Context "$OS Test- package cmdlets" {
                $mofPath = "$outputFolder/DscConfig$OS.mof"
                It 'validates the package generated by New-GuestConfigurationPackage' {
                        ...
                        Mock -CommandName 'Get-GuestConfigBinaryPath' -ModuleName 'GuestConfigPath' -MockWith { "$Env:BuildTempFolder/guestconfigurationtest/bin/DSC/" } -Verifiable
