...
'@                
    ...
    InModuleScope -ModuleName 'GuestConfiguration' {
        ...
        It 'Verify Test-GuestConfigurationPackage can validate the package generated by New-GuestConfigurationPackage (Windows only)' {
            if ($isWindows) {
                Mock -CommandName 'Get-GuestConfigPath' -MockWith { "$env:Temp/guestconfigurationtest/" } -Verifiable
                Mock -CommandName 'Get-GuestConfigPath' -ModuleName 'GuestConfiguration' -MockWith { "$env:Temp/guestconfigurationtest/" } -Verifiable
                
                # it should not be neccessary to mock these cmdlets
                # test quality can be improved by mocking the path for publish
                ...
                Mock -CommandName 'Test-DscConfiguration'  -ModuleName 'GuestConfiguration' -MockWith { @{compliance_state=$false;resources_in_desired_state = @();resources_not_in_desired_state=@('TimeZoneExample')} } -Verifiable
