...
'@                
        ...
        # Extract agent files
        If ($IsWindows) {
            Expand-Archive $PSScriptRoot/../bin/DSC_Windows.zip "$outputFolder/guestConfig/bin/" -Force
        }
        else {
            Expand-Archive $PSScriptRoot/../bin/DSC_Linux.zip "$outputFolder/guestConfig/bin/" -Force
        }
    ...
    InModuleScope -ModuleName 'GuestConfiguration' {
        ...
        It 'Verify Test-GuestConfigurationPackage can validate the package generated by New-GuestConfigurationPackage (Windows only)' {
            if ($isWindows) {
                $packageFolder = New-Item "$outputFolder/package" -ItemType 'directory' | ForEach-Object {$_.FullName}

                Mock Get-GuestConfigPath -ModuleName GuestConfigPath -MockWith { "$env:Temp/guestconfigurationtest/" }

                New-GuestConfigurationPackage -Configuration $mofPath -Name $policyName -Path $packageFolder
                $result = Test-GuestConfigurationPackage -Path $packageFolder -Verbose
