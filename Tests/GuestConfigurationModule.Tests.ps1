    ...
    InModuleScope -ModuleName 'GuestConfiguration' {
        ...
        foreach ($OS in @('Windows','Linux')) {
            Context "$OS Package creation" {
                ...
                if ('Linux' -eq $OS) {
                    ...
                    It 'contains both copies of the native InSpec resource' {
                        ...
                        $nativeInSpecResource.Count | Should -Be 2
                    }
                }
            }
        }
        ...
        Context 'Policy cmdlets' {
            ...
            It 'Verify New-GuestConfigurationPolicy cmdlet can create custom policy definitions' {
                ...
                $jsonDefinition.properties.policyRule.then.details.deployment.properties.template.parameters.type | Should -Be "@{type=string}"
