    ...
    InModuleScope -ModuleName 'GuestConfiguration' {
        ...
        It 'Verify Test-GuestConfigurationPackage can validate the package generated by New-GuestConfigurationPackage (Windows only)' {
            if ($isWindows) {
                ...
                Mock -CommandName 'Publish-DscConfiguration' -ModuleName 'DscOperations'
                Mock -CommandName 'Test-DscConfiguration'  -ModuleName 'DscOperations' -MockWith { @{resources_in_desired_state = '';resources_not_in_desired_state=''} }
                Mock -CommandName 'Get-DscConfiguration' -ModuleName 'DscOperations'
