...
<#
    .SYNOPSIS
        Creates a common 'IF' section for policy definitions
#>

function New-GuestConfigurationPolicyIfSection {
    [CmdletBinding()]
    param
    (
        [Parameter(Mandatory = $true)]
...
<#
    .SYNOPSIS
        Creates a new deploy policy definition for a guest configuration policy.
#>
...
function New-GuestConfigurationPolicyDefinition {
    ...
    foreach ($currentPolicyInfo in $CustomPolicyParameters) {
        $currentPolicyInfo['FolderPath'] = $PolicyFolderPath
...
function New-CustomGuestConfigPolicy {
    ...
    $existingPolicy = $existingPolicies | Where-Object { ($_.Properties.PSObject.Properties.Name -contains 'displayName') -and ($_.Properties.displayName -eq $CustomPolicyParameters.DisplayName) }
    if ($null -ne $existingPolicy) {
        Write-Verbose -Message "Found policy with name '$($existingPolicy.Properties.displayName)' and guid '$($existingPolicy.Name)'..."
        $CustomPolicyParameters['Guid'] = $existingPolicy.Name
    }
